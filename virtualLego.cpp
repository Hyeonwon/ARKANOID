...
// add a ball and change its position
const float spherePos[brick_ball][2] = {
	// Top border
	{3.5, -1.5}, {3.5, -1.0}, {3.5, -0.5}, {3.5, 0}, {3.5, 0.5}, {3.5, 1.0}, {3.5, 1.5},

	// Bottom border
	{-1.5, -1.5}, {-1.5, -1.0}, {-1.5, -0.5}, {-1.5, 0}, {-1.5, 0.5}, {-1.5, 1.0}, {-1.5, 1.5},

	// Right border
	{0.5, -2.0}, {0, -2.0}, {-1.0, -2.0}, {-0.5, -2.0}, {3.0, -2.0}, {2.5, -2.0}, {2.0, -2.0}, {1.5, -2.0}, {1.0, -2.0},

	// Left border
	{3.0, 2.0}, {2.5, 2.0}, {2.0, 2.0}, {1.5, 2.0}, {1.0, 2.0}, {0.5, 2.0}, {0, 2.0}, {-1.0, 2.0}, {-0.5, 2.0},

	// Eyes
	{2.5, 1.0}, {2.5, -1.0}, {2.0, 1.0}, {2.0, -1.0},

	// Nose
	{1.5, 0}, {1.0, 0},

	// Mouth
	{0.25, 1.25}, {-0.25, 1.0}, {0.25, -1.25}, {-0.25, -1.0}, {-0.5, 0.5}, {-0.5, 0}, {-0.5, -0.5}
};
...
public:
	...
	void hitBy(CSphere& ball) // If the balls hit each other
	{
		if (hasIntersected(ball)) {
			if (ball.getIsControlBall()) {
				if (this->getVelocity_Z() > 0) // Set it to go up
			...
			if (this->getVelocity_Z() > 0) // Set it to the downward direction
				this->setPower(-this->getVelocity_Z(), this->getVelocity_X());
			else
				this->setPower(this->getVelocity_Z(), -this->getVelocity_X());
...
public:
	...
	void hitBy(CSphere& ball) // When I hit the wall and the ball
	{
		if (hasIntersected(ball)) {
...
bool Setup()
{
	...
	if (false == g_legowall[3].create(Device, -1, -1, 0, 0, 0, d3d::DARKRED)) return false; // ball set to zero, such as moving surface height, width, etc
	g_legowall[3].setPosition(-4.56f, 0.12f, 0.0f);


	for (i = 0; i < brick_ball; i++) { // Add the number of balls, select the location
